...
function crc32HashString(const aString: string; aIgnoreCase, aIgnoreBlanks: boolean): pointer;
...
function crc32HashString(const aString: string; aIgnoreCase, aIgnoreBlanks: boolean): pointer;
  function crc32(p: pchar; aLenght: integer): dword;
  var
    x: integer;
  begin
    result := $FFFFFFFF;
    for x := 0 to aLenght - 1 do
    begin
      result := (result shr 8)
            xor crc32table [pbyte(p)^ xor (result and $000000ff)];
      Inc(p);
    end;
    result := not result;
  end;
var
  src, pdst, dst, len: integer;
  xString: string;
begin
  xString := aString;
  if aIgnoreBlanks then
  begin
    src := 1;
    pdst := 1;
    dst := 1;
    ...
    while src <= len do
    begin
      if (   (not (aString[src] in [#9,#32]))
          or (not (aString[pdst] in [#9,#32]))
         ) then
      begin
        xString[dst] := aString[src];
        pdst := dst;
        Inc (dst);
      end;
      Inc (src);
    end;
    SetLength (xString, dst - 1);
  end;
  if aIgnoreCase then
    xString := AnsiLowerCase(xString);
  result := pointer (crc32 (pchar(xString), length(xString)));
end;
