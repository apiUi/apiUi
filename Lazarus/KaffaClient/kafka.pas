...
uses
  Classes, SysUtils, ctypes, dynlibs, winsock;
...
type Trd_kafka_topic_new = function (rk: Prd_kafka_t; topic: PChar; conf: Prd_kafka_topic_conf_t): Prd_kafka_topic_t; cdecl;
type Trd_kafka_flush = function (rk: Prd_kafka_t; timeout_ms: Int32): Trd_kafka_resp_err_t; cdecl;
type Trd_kafka_topic_destroy = procedure (rkt: Prd_kafka_topic_t); cdecl;
type Trd_kafka_destroy = procedure (rk: Prd_kafka_t); cdecl;
type Trd_kafka_version_str = function: PChar; cdecl;
type Trd_kafka_last_error = function: Trd_kafka_resp_err_t; cdecl;
type Trd_kafka_err2str = function (err: Trd_kafka_resp_err_t): PChar; cdecl;
type Trd_kafka_err2name = function (err: Trd_kafka_resp_err_t): PChar; cdecl;
type Trd_kafka_conf_set = function (conf: Prd_kafka_conf_t; name: PChar; value: PChar; errstr: PChar; errstr_size: size_t): Trd_kafka_conf_res_t; cdecl;
type Trd_kafka_topic_name = function (rkt: Prd_kafka_topic_t): PChar; cdecl;
type Trd_kafka_conf_new = function: Prd_kafka_conf_t; cdecl;
type FARPROC = Pointer;


var
  rd_kafka_new: Trd_kafka_new;
  rd_kafka_produce:Trd_kafka_produce;
  rd_kafka_poll: Trd_kafka_poll;
  rd_kafka_topic_new: Trd_kafka_topic_new;
  rd_kafka_flush: Trd_kafka_flush;
  rd_kafka_topic_destroy: Trd_kafka_topic_destroy;
  rd_kafka_destroy: Trd_kafka_destroy;
  rd_kafka_version_str: Trd_kafka_version_str;
  rd_kafka_last_error: Trd_kafka_last_error;
  rd_kafka_err2str: Trd_kafka_err2str;
  rd_kafka_err2name: Trd_kafka_err2name;
  rd_kafka_conf_set: Trd_kafka_conf_set;
  rd_kafka_topic_name: Trd_kafka_topic_name;
  rd_kafka_conf_new: Trd_kafka_conf_new;
  rd_kafka_handle: THandle;

implementation

function CheckProcAddress(AName: PChar): FARPROC;
begin
  Result := GetProcAddress(rd_kafka_handle, AName);
  if Result=nil then
    raise Exception.Create (Format('The procedure "%s" in dll "%s" can not be found.', [AName, RD_EXPORT]));
end;

initialization
  rd_kafka_handle := 0;
  rd_kafka_handle := LoadLibrary(RD_EXPORT);
  if rd_kafka_handle <> 0 then
  begin
    rd_kafka_new := CheckProcAddress('rd_kafka_new');
    rd_kafka_produce := CheckProcAddress('rd_kafka_produce');
    rd_kafka_poll := CheckProcAddress('rd_kafka_poll');
    rd_kafka_topic_new := CheckProcAddress('rd_kafka_topic_new');
    rd_kafka_flush := CheckProcAddress('rd_kafka_flush');
    rd_kafka_topic_destroy := CheckProcAddress('rd_kafka_topic_destroy');
    rd_kafka_destroy := CheckProcAddress('rd_kafka_destroy');
    rd_kafka_version_str := CheckProcAddress('rd_kafka_version_str');
    rd_kafka_last_error := CheckProcAddress('rd_kafka_last_error');
    rd_kafka_err2str := CheckProcAddress('rd_kafka_err2str');
    rd_kafka_err2name := CheckProcAddress('rd_kafka_err2name');
    rd_kafka_conf_set := CheckProcAddress('rd_kafka_conf_set');
    rd_kafka_topic_name := CheckProcAddress('rd_kafka_topic_name');
    rd_kafka_conf_new := CheckProcAddress('rd_kafka_conf_new');
  end;

finalization
  if rd_kafka_handle <> 0 then
  begin
    UnloadLibrary(rd_kafka_handle);
  end;
